    ...
    >
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center'>
        ...
        <nav className='hidden lg:flex items-center space-x-8'>
          <button
            onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
            className={`text-sm font-semibold transition-colors tracking-wide cursor-pointer ${
              scrolled
        ...
        <div className='hidden lg:flex items-center space-x-6'>
          <button
            onClick={() => scrollToSection('contato')}
            className={`px-6 py-2.5 rounded-full font-semibold text-sm tracking-wide transition-all transform hover:-translate-y-0.5 cursor-pointer ${
              scrolled
                ? 'bg-primary hover:bg-primary-dark text-white shadow-md shadow-primary/30'
                : 'bg-white/10 hover:bg-white/20 text-white border border-white/30 shadow-none'
            }`}
          >
            AGENDE UMA VISITA
          </button>
      ...
      {/* Mobile Nav Overlay */}
      {isMobileMenuOpen && (
        <div className='lg:hidden absolute top-full left-0 right-0 bg-white/95 backdrop-blur-sm shadow-xl border-t border-gray-100 flex flex-col py-4 px-6 space-y-4'>
          <button
          ...
          <button onClick={() => scrollToSection('infraestrutura')} className='text-left py-2 font-semibold text-gray-800 hover:text-primary cursor-pointer'>INFRAESTRUTURA</button>
